        ...
        dataflow: function (options) {
            ...
            var handlerFactory = {
                initHtml: function (html, data) {
                    ...
                    var render = window.template.compile(html)
                    var ret = {
                        id: '#' + data.id,
                        str: render(data)
                    }
                ...
                */
                generateNodeId: function () {
                    ...
                    var uuid = 'xxxxxx'.replace(/[xy]/g, c => {
            ...
            var _initDragging = function () {
                ...
                _self
                    .on('mousedown', opts.tools, function (e) {
                        ...
                        var _html = handlerFactory.getLocationTemplate(type)
                ...
                $(opts.canvas).on('mouseup', '.bk-flow-location', function (e) {
                    (opts.onLocationMoveAfter && 
                        opts.onLocationMoveAfter($(e.target).closest('.bk-flow-location').attr('id')))
                })
            }
            ...
            var _initEvent = function () {
                ...
                instance.bind('connectionDragStop', function (connection) {
                    ...
                    // onLineDragStop 先判断是否删除添加的连线
                    if ((opts.onLineDragStop && !opts.onLineDragStop(_line)) ||
            ...
            var _createLocation = function (location) {
                ...
                var _html = handlerFactory.getLocationTemplate(location.type)
            ...
            var _updateLocationeById = function (id, data) {
                ...
                var _html = handlerFactory.getLocationTemplate(rawData.type)
            ...
            var _getLocationById = function (id) {
                var $location = $('#' + id)
                var rawData = $location.data('raw')
                // 若画布存在拖拽，则获取节点位移时需加上位移距离
                rawData.x = parseInt($location.css('left')) + $nodeContainer.left
                rawData.y = parseInt($location.css('top')) + $nodeContainer.top
                return rawData
            }
